...
export default function IdeaProject() {
	...
	const Circle = ({ x, y, size, color }: { x: number, y: number, size: number, color: string }) => {
		...
		const offsetX = useSpring(useTransform(mouseX, (value) => (value - (windowWidth || 0) / 2) * 0.02), springConfig);
		const offsetY = useSpring(useTransform(mouseY, (value) => (value - (windowHeight || 0) / 2) * 0.02), springConfig);
